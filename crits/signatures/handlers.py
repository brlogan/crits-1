...
def handle_signature_file(data, source_name, user=None,
    ...
    if isinstance(source_name, basestring) and len(source_name) > 0:
        ...
        signature.add_source(source_name, method=method, reference=reference, tlp=source_tlp)
    elif isinstance(source_name, list) and len(source_name) > 0:
        for s in source_name:
            if isinstance(s, EmbeddedSource):
                signature.add_source(s, method=method, reference=reference, tlp=source_tlp)
